import { NextRequest } from "next/server";
import { z } from "zod";
import { getPagination, handleApi } from "@/lib/api";
import { requirePermission } from "@/lib/auth";
import { prisma } from "@/lib/db";
import { writeAudit } from "@/lib/audit";
import { makeSlug, uniqueSlug } from "@/lib/slug";

export const runtime = "nodejs";

const gallerySchema = z.object({
  title: z.string().min(1),
  slug: z.string().optional().nullable(),
  description: z.string().optional().nullable(),
  coverUrl: z.string().optional().nullable(),
  authorId: z.string().optional().nullable(),
  status: z.string().optional(),
  items: z
    .array(
      z.object({
        mediaId: z.string().optional().nullable(),
        imageUrl: z.string().min(1),
        caption: z.string().optional().nullable(),
        credit: z.string().optional().nullable(),
        sortOrder: z.number().int().optional(),
      }),
    )
    .optional(),
});

export async function GET(req: NextRequest) {
  return handleApi(async () => {
    const url = new URL(req.url);
    const publicOnly = url.searchParams.get("public") === "1";
    if (!publicOnly) await requirePermission("galleries", "read");
    const { skip, take, page, pageSize } = getPagination(url);
    const where = {
      status: publicOnly ? "published" : url.searchParams.get("status") || undefined,
    };
    const [total, items] = await Promise.all([
      prisma.gallery.count({ where }),
      prisma.gallery.findMany({
        where,
        include: {
          author: true,
          items: { orderBy: { sortOrder: "asc" } },
          _count: { select: { items: true } },
        },
        orderBy: { createdAt: "desc" },
        skip,
        take,
      }),
    ]);
    return { items, total, page, pageSize };
  });
}

export async function POST(req: NextRequest) {
  return handleApi(async () => {
    const user = await requirePermission("galleries", "create");
    const body = gallerySchema.parse(await req.json());
    const baseSlug = makeSlug(body.slug || body.title);
    const existing = await prisma.gallery.findMany({
      where: { slug: { startsWith: baseSlug } },
      select: { slug: true },
    });
    const slug = uniqueSlug(
      baseSlug,
      existing.map((e) => e.slug),
    );
    const item = await prisma.gallery.create({
      data: {
        title: body.title,
        slug,
        description: body.description,
        coverUrl: body.coverUrl,
        authorId: body.authorId,
        status: body.status || "draft",
        items: body.items?.length
          ? {
              create: body.items.map((it, idx) => ({
                mediaId: it.mediaId,
                imageUrl: it.imageUrl,
                caption: it.caption,
                credit: it.credit,
                sortOrder: it.sortOrder ?? idx,
              })),
            }
          : undefined,
      },
      include: { items: { orderBy: { sortOrder: "asc" } } },
    });
    await writeAudit({ userId: user.id, action: "create", module: "galleries", recordId: item.id });
    return item;
  });
}
